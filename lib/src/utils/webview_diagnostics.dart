...
class WebViewDiagnostics {
  static Future<void> testCheckoutUrl(String cartId) async {
    final url =
        '${AppConfig.apiBaseUrl}/api/v1/ecommerce/checkout/cart/$cartId';
    ...
    try {
      ...
      final baseResponse = await http
          .get(
            ...
            headers: {'User-Agent': 'Soujanya-App-Diagnostics/1.0'},
          )
      ...
      final checkoutResponse = await http
          .get(
            ...
            headers: {
              ...
              'User-Agent': 'Soujanya-App-Diagnostics/1.0',
            },
          )
      ...
      if (checkoutResponse.statusCode == 200) {
        ...
        if (checkoutResponse.body.toLowerCase().contains('<html')) {
          ...
          debugPrint(
            'Response preview: ${checkoutResponse.body.substring(0, 200)}...',
          );
        }
      } else {
        debugPrint(
          '❌ Checkout URL returned error: ${checkoutResponse.statusCode}',
        );
  ...
  static void logWebViewSettings() {
    ...
    debugPrint(
      'User Agent: Mozilla/5.0 (Linux; Android 10; Mobile) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.120 Mobile Safari/537.36 Soujanya-App/1.0',
    );
