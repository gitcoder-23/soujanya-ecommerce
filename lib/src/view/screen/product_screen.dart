...
class _ProductScreenState extends State<ProductScreen> {
  ...
  Widget _buildGridView() {
    return GridView.builder(
      ...
      gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
        crossAxisCount: 2,
        childAspectRatio: 0.62, // Adjusted for better content display
        crossAxisSpacing: 16,
        mainAxisSpacing: 16,
      ),
      itemCount:
          _products.length +
          (_isLoadingMore ? 2 : 0), // Add 2 skeleton items when loading more
  ...
  Widget _buildListView() {
    return ListView.builder(
      ...
      itemCount:
          _products.length +
          (_isLoadingMore ? 1 : 0), // Add 1 skeleton item when loading more
      itemBuilder: (context, index) {
        if (index >= _products.length) {
          // Show skeleton loading for additional items
          return Container(
          ...
          child: ProductCard(
            ...
            onTap: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => ProductDetailScreen(product: {'slug': product.slug}),
                ),
              );
            },
          ),
  ...
  Widget _buildGridSkeleton() {
    return GridView.builder(
      ...
      gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
        crossAxisCount: 2,
        childAspectRatio: 0.62, // Match the actual grid aspect ratio
        crossAxisSpacing: 16,
        mainAxisSpacing: 16,
      ),
