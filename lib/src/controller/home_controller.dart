...
class HomeController extends GetxController {
  ...
  Future<void> loadLatestBlogPosts() async {
    blogLoading.value = true;
    blogError.value = null;
    try {
      final response = await _blogService.getBlogPosts(page: 1, perPage: 3);
      final posts = response['data'] as List<BlogPost>;
      latestBlogPosts.assignAll(posts);
    } catch (e) {
      // Set error for display, except for maintenance/network errors
      if (e is! MaintenanceException &&
          e is! NoInternetException &&
          e is! ServerErrorException) {
        blogError.value = e.toString();
        latestBlogPosts.assignAll([]);
      }
    } finally {
