...
import 'package:martfury/src/constants/coupon_error_codes.dart';
...
class _CartScreenState extends State<CartScreen> {
  ...
  void _setupCurrencyChangeListener() {
    // Listen to currency changes to refresh cart data with new currency formatting
    _currencyChangeSubscription = CurrencyChangeService.currencyChangeStream.listen((newCurrency) {
      if (mounted) {
        // Reload cart data to get updated prices in new currency
        _loadCartDetail(showLoading: false);
      }
    });
  }
  ...
  void _setupLanguageChangeListener() {
    // Listen to language changes to refresh cart data with new language
    _languageChangeSubscription = LanguageChangeService.languageChangeStream.listen((newLanguage) {
      if (mounted) {
        // Reload cart data to get updated text in new language
        _loadCartDetail(showLoading: false);
      }
    });
  }
  ...
  Future<void> _loadCartDetail({bool showLoading = true}) async {
    ...
    try {
      ...
      // The backend handles coupon storage with the cart
      ...
      if (mounted) {
        // Check if the error is due to cart being cleared
        if (e.toString().contains('Cart not found. Cart has been cleared.')) {
          setState(() {
            _cartDetail = {};
            _isLoading = false;
            _error = '';
  ...
  Widget build(BuildContext context) {
    return BackButtonHandler.createModalPopScope(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        leading: IconButton(
          icon: Icon(Icons.arrow_back_ios_new, color: AppColors.appBarForeground),
          onPressed: () {
        ...
        actions: [
          IconButton(
            icon: Icon(Icons.search, color: AppColors.appBarForeground),
            onPressed: () {
                              ...
                              child: Column(
                                children: [
                                                  ...
                                                  try {
                                                    ...
                                                    if (cartId != null) {
                                                      ...
                                                      if (response['error'] == false || response['error'] == null) {
                                                        if (state.mounted) {
                                                          scaffoldMessenger
                                                              .showSnackBar(
                                                                SnackBar(
                                                                  content: Text(
                                                                    response['message'] ?? 'cart.coupon_applied'.tr(),
                                                                  ),
                                                                  backgroundColor:
                                                                      AppColors
                                                                          .success,
                                                        ...
                                                        if (state.mounted) {
                                                          final errorCode = response['data']?['error_code'] ?? response['error_code'];
                                                          final errorMessage = CouponErrorCodes.getErrorMessage(
                                                            errorCode, 
                                                            response['message']
                                                          );
                                                    ...
                                                    if (state.mounted) {
                                                      ...
                                                      if (errorMessage.startsWith('Exception: Failed to apply coupon: Exception: ')) {
                                                        errorMessage = errorMessage.substring('Exception: Failed to apply coupon: Exception: '.length);
                                                      } else if (errorMessage.startsWith('Exception: Failed to apply coupon: ')) {
                                                        errorMessage = errorMessage.substring('Exception: Failed to apply coupon: '.length);
                                  ...
                                  Container(
                                    ...
                                    child: Column(
                                      children: [
                                        // Subtotal row - only show if sub_total_formatted is not null
                                        if (_cartDetail['sub_total_formatted'] != null) ...[
                                          Row(
                                            mainAxisAlignment:
                                        ...
                                        if (_cartDetail['tax_amount'] != null &&
                                            (_cartDetail['tax_amount'] as num) >
                                                0) ...[
                                          // Add spacing only if subtotal was shown, otherwise this is the first item
                                          if (_cartDetail['sub_total_formatted'] != null)
                                            const SizedBox(height: 8),
                                          Row(
                                            mainAxisAlignment:
                                        ...
                                        if (_cartDetail['applied_coupon_code'] !=
                                            null) ...[
                                          // Add spacing only if subtotal or tax was shown, otherwise this is the first item
                                          if (_cartDetail['sub_total_formatted'] != null ||
                                              (_cartDetail['tax_amount'] != null &&
                                                  (_cartDetail['tax_amount'] as num) > 0))
                                            const SizedBox(height: 8),
                                          Row(
                                            mainAxisAlignment:
                                                MainAxisAlignment.spaceBetween,
                                              ...
                                              Row(
                                                children: [
                                                  ...
                                                  IconButton(
                                                    ...
                                                    onPressed: () => _showRemoveCouponConfirmation(),
                                                  ),
                                                ],
                                              ),
                                            ],
                                        ...
                                        // Only show divider and spacing if there are any items above (subtotal, tax, or coupon)
                                        if (_cartDetail['sub_total_formatted'] != null ||
                                            (_cartDetail['tax_amount'] != null &&
                                                (_cartDetail['tax_amount'] as num) > 0) ||
                                            _cartDetail['applied_coupon_code'] != null) ...[
                                          const SizedBox(height: 8),
                                          const Divider(),
                                          const SizedBox(height: 8),
                                        ],
  ...
  Future<void> _showRemoveCouponConfirmation() async {
    final shouldRemove = await showDialog<bool>(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Text(
            'cart.remove_coupon_confirm_message'.tr(),
            style: kAppTextStyle(
              fontSize: 16,
              color: AppColors.getSecondaryTextColor(context),
            ),
  ...
  Future<void> _removeCoupon() async {
    ...
    try {
      final cartId = await CartService.getCartId();
      if (cartId != null) {
        final response = await _cartService.removeCoupon(cartId: cartId);
        if (state.mounted) {
          scaffoldMessenger.showSnackBar(
            SnackBar(
              content: Text(response['message'] ?? 'cart.coupon_removed'.tr()),
              backgroundColor: AppColors.success,
            ),
          );
          _loadCartDetail(showLoading: false);
  ...
  Future<void> _handleCheckoutPressed() async {
    ...
    if (AppConfig.enableGuestCheckout) {
      // Guest checkout is enabled, proceed directly to checkout
      await _proceedToCheckout();
    } else {
      // Guest checkout is disabled, check if user is logged in
  ...
  Future<void> _proceedToCheckout() async {
    ...
    if (cartId != null && mounted) {
      navigator.push(
        MaterialPageRoute(
          builder: (context) => CheckoutScreen(cartId: cartId),
        ),
      );
    }
  }
